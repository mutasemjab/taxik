                ...
                <form action="{{ route('wallet_transactions.store') }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="entity_type" name="entity_type" required>
                                    ...
                                    <option value="driver" {{ old('entity_type') == 'driver' ? 'selected' : '' }}>
                                        {{ __('messages.Driver') }}</option>
                                </select>
                            </div>

                            <!-- User Selection -->
                            <div class="form-group entity-select user-select"
                                ...
                                <select class="form-control" id="user_entity_id" name="entity_id_user">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            ...
                                            {{ $user->name }} ({{ $user->phone }}) - {{ __('messages.Balance') }}:
                                            {{ $user->balance }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" id="driver_entity_id" name="entity_id_driver">
                                    ...
                                    @foreach ($drivers as $driver)
                                        <option value="{{ $driver->id }}"
                                            {{ old('entity_id') == $driver->id && old('entity_type') == 'driver' ? 'selected' : '' }}
                                            data-balance="{{ $driver->balance }}">
                                            {{ $driver->name }} ({{ $driver->phone }}) - {{ __('messages.Balance') }}:
                                            {{ $driver->balance }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <!-- Hidden field for actual entity_id -->
                        ...
                        <div class="col-md-6">
                            <!-- Transaction Details -->
                            <div class="form-group">
                                <label for="type_of_transaction">{{ __('messages.Transaction_Type') }} <span
                                        class="text-danger">*</span></label>
