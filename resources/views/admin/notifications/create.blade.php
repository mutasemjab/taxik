                        ...
                        <form action="{{ route('notifications.send') }}" method="post" id="notification-form">
                            ...
                            <div class="form-group">
                                ...
                                <select name="type" id="type" class="form-control select2"
                                        data-placeholder="{{ __('messages.Notification_Type') }}">
                            ...
                            <div class="form-group" id="specific-user-field" style="display:none;">
                                ...
                                <select name="target_id" id="target_id_user"
                            ...
                            <div class="form-group" id="specific-driver-field" style="display:none;">
                                ...
                                <select name="target_id" id="target_id_driver"
                                        class="form-control select2"
...
$(document).ready(function () {
    ...
    function toggleTargetField() {
        ...
        var showUser   = (type === '3');
        var showDriver = (type === '4');

        $('#specific-user-field').toggle(showUser);
        $('#specific-driver-field').toggle(showDriver);

        // عند الإظهار نعيد تهيئة select2 عشان يحسب العرض صح
        if (showUser) {
            $('#target_id_user').select2({
                theme: 'bootstrap4',
                width: '100%',
                allowClear: true,
        ...
        if (showDriver) {
            $('#target_id_driver').select2({
                theme: 'bootstrap4',
                width: '100%',
                allowClear: true,
                placeholder: '{{ __("messages.Select_Driver") }}'
            });
        }

        // حقل name: فقط الظاهر يرسل البيانات
        $('#target_id_user').attr('name',   showUser   ? 'target_id' : '');
        $('#target_id_driver').attr('name', showDriver ? 'target_id' : '');
    }

    // تشغيل عند تغيير النوع
    $('#type').on('change', toggleTargetField);

    // تشغيل عند تحميل الصفحة (لاستعادة الحالة بعد validation error)
    toggleTargetField();
});
</script>
@endsection
