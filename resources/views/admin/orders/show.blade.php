    ...
    <div class="container-fluid">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-8">
                        ...
                        <div class="timeline">
                            ...
                            <div
                                class="timeline-item {{ in_array($statusValue, ['accepted', 'on_the_way', 'arrived', 'started', 'waiting_payment', 'completed']) ? 'completed' : '' }}">
                                <i class="fas fa-check"></i> {{ __('messages.Accepted') }}
                            </div>
                            <div
                                class="timeline-item {{ in_array($statusValue, ['on_the_way', 'arrived', 'started', 'waiting_payment', 'completed']) ? 'completed' : '' }}">
                                <i class="fas fa-car"></i> {{ __('messages.On_The_Way') }}
                            </div>
                            <div
                                class="timeline-item {{ in_array($statusValue, ['arrived', 'started', 'waiting_payment', 'completed']) ? 'completed' : '' }}">
                                <i class="fas fa-map-marker-alt"></i> {{ __('messages.Arrived') }}
                            </div>
                            <div
                                class="timeline-item {{ in_array($statusValue, ['started', 'waiting_payment', 'completed']) ? 'completed' : '' }}">
                                <i class="fas fa-play"></i> {{ __('messages.Started') }}
                            </div>
                            <div
                                class="timeline-item {{ in_array($statusValue, ['waiting_payment', 'completed']) ? 'completed' : '' }}">
                        ...
                        <form action="{{ route('orders.updateStatus', $order->id) }}" method="POST" class="w-100">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="status" name="status">
                                    ...
                                    <option value="on_the_way" {{ $statusValue == 'on_the_way' ? 'selected' : '' }}>
                                        {{ __('messages.On_The_Way') }}</option>
                                    <option value="arrived" {{ $statusValue == 'arrived' ? 'selected' : '' }}>
                                        {{ __('messages.Arrived') }}</option>
                                    <option value="started" {{ $statusValue == 'started' ? 'selected' : '' }}>
                                        {{ __('messages.Started') }}</option>
                                    <option value="waiting_payment"
                                        {{ $statusValue == 'waiting_payment' ? 'selected' : '' }}>
                                        {{ __('messages.Waiting_Payment') }}</option>
                                    <option value="completed" {{ $statusValue == 'completed' ? 'selected' : '' }}>
                                        {{ __('messages.Completed') }}</option>
                                    <option value="user_cancel_order"
                                        {{ $statusValue == 'user_cancel_order' ? 'selected' : '' }}>
                                        {{ __('messages.User_Cancelled') }}</option>
                                    <option value="driver_cancel_order"
                                        {{ $statusValue == 'driver_cancel_order' ? 'selected' : '' }}>
                                        {{ __('messages.Driver_Cancelled') }}</option>
                                    <option value="cancel_cron_job"
                                        {{ $statusValue == 'cancel_cron_job' ? 'selected' : '' }}>
                ...
                @if (in_array($statusValue, ['user_cancel_order', 'driver_cancel_order', 'cancel_cron_job']) &&
                    ...
                    <div class="col-md-3">
                        <div class="card bg-light mb-3">
                            <div class="card-body text-center">
                                ...
                                <h4 class="mb-0">{{ number_format($order->live_distance, 2) }} {{ __('messages.KM') }}
                                </h4>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card bg-light mb-3">
                            <div class="card-body text-center">
                                ...
                                <h4 class="mb-0">{{ $order->actual_trip_duration_minutes ?? 'N/A' }}
                                    {{ __('messages.Minutes') }}</h4>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="card bg-light mb-3">
                            <div class="card-body text-center">
                                ...
                                <h4 class="mb-0 text-success">{{ number_format($order->returned_amount ?? 0, 2) }}
                                    {{ __('messages.JD') }}</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <table class="table table-bordered">
                        <tbody>
                            <tr>
                                ...
                                <td>{{ $order->trip_started_at ? $order->trip_started_at->format('Y-m-d H:i:s') : __('messages.Not_Started') }}
                                </td>
                            </tr>
                            <tr>
                                <th>{{ __('messages.Trip_Completed_At') }}</th>
                                <td>{{ $order->trip_completed_at ? $order->trip_completed_at->format('Y-m-d H:i:s') : __('messages.Not_Completed') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                            ...
                            <table class="table table-bordered">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-success px-3 py-2">
                                                {{ number_format($order->in_trip_waiting_charges, 2) }}
                                                {{ __('messages.JD') }}
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge badge-primary px-3 py-2">
                                                {{ number_format($order->waiting_charges + $order->in_trip_waiting_charges, 2) }}
                                                {{ __('messages.JD') }}
                                            </span>
                                        </td>
                                    </tr>
                    ...
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <p>{{ $order->pick_name }}</p>
                                <small class="text-muted">{{ __('messages.Coordinates') }}: {{ $order->pick_lat }},
                                    {{ $order->pick_lng }}</small>
                            </div>
                            <div class="col-md-6">
                                ...
                                @if ($order->drop_lat && $order->drop_lng)
                                    <small class="text-muted">{{ __('messages.Coordinates') }}: {{ $order->drop_lat }},
                                        {{ $order->drop_lng }}</small>
                                @endif
                            </div>
                        </div>
                                    ...
                                    <div class="card-body p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-4 text-right">
                                                <h6 class="mb-0">
                                                    {{ number_format($order->total_price_before_discount, 2) }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div class="row">
                                                <div class="col-8">
                                                    <h6 class="card-title mb-0 text-success">
                                                        {{ __('messages.Discount') }}</h6>
                                                </div>
                                                <div class="col-4 text-right">
                                                    <h6 class="mb-0 text-success">
                                                        -{{ number_format($order->discount_value, 2) }}</h6>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="card mb-3 bg-primary text-white">
                                    <div class="card-body p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-4 text-right">
                                                <h6 class="mb-0">
                                                    {{ number_format($order->total_price_after_discount, 2) }}</h6>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card mb-3">
                                    <div class="card-body p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-4 text-right">
                                                <h6 class="mb-0">{{ number_format($order->net_price_for_driver, 2) }}
                                                </h6>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="card mb-3">
                                    <div class="card-body p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-4 text-right">
                                                <h6 class="mb-0">{{ number_format($order->commision_of_admin, 2) }}
                                                </h6>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card-body p-3">
                                        <div class="row">
                                            ...
                                            <div class="col-md-6 text-right">
                                                ...
                                                <div class="mt-1">
                                                    <span
                                                        class="badge px-3 py-2 {{ $paymentStatus == 'paid' ? 'badge-success' : 'badge-warning' }}">
                        ...
                        @if ($order->user)
                            <div class="text-center mb-3">
                                @if ($order->user->photo)
                                    <img src="{{ asset('assets/admin/uploads/' . $order->user->photo) }}"
                                        alt="{{ $order->user->name }}" class="img-profile rounded-circle mb-3"
                            ...
                            <ul class="list-group mb-3">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <span
                                        class="badge badge-primary px-3 py-2">{{ number_format($order->user->balance, 2) }}</span>
                                </li>
                            </ul>
                        ...
                        @if ($order->driver)
                            <div class="text-center mb-3">
                                @if ($order->driver->photo)
                                    <img src="{{ asset('assets/admin/uploads/' . $order->driver->photo) }}"
                                        alt="{{ $order->driver->name }}" class="img-profile rounded-circle mb-3"
                                        style="width: 100px; height: 100px; object-fit: cover;">
                                @else
                                    <img src="{{ asset('assets/admin/img/undraw_profile.svg') }}" alt="No Image"
                                        class="img-profile rounded-circle mb-3"
                            ...
                            <ul class="list-group mb-3">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    {{ __('messages.Wallet_Balance') }}
                                    <span
                                        class="badge badge-primary px-3 py-2">{{ number_format($order->driver->balance, 2) }}</span>
                                </li>
                            </ul>
