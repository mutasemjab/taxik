...
@section('title', __('messages.Ban_User'))
...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-ban text-danger"></i> {{ __('messages.Ban_User') }}
        </h1>
        ...
        <div class="col-md-4">
            <div class="card shadow mb-4">
                ...
                <div class="card-body text-center">
                    @if($user->photo)
                    <img src="{{ asset('assets/admin/uploads/' . $user->photo) }}" alt="{{ $user->name }}" class="img-fluid rounded-circle mb-3" style="width: 150px; height: 150px; object-fit: cover;">
                    @else
                    ...
                    <div class="text-left">
                        <p class="mb-2"><strong>{{ __('messages.Car_Model') }}:</strong> {{ $user->model ?? __('messages.N/A') }}</p>
                        <p class="mb-2"><strong>{{ __('messages.Plate_Number') }}:</strong> {{ $user->plate_number ?? __('messages.N/A') }}</p>
                        <p class="mb-2"><strong>{{ __('messages.Balance') }}:</strong> {{ $user->balance }}</p>
                        <p class="mb-2">
            ...
            <div class="card shadow mb-4">
                <div class="card-header py-3 bg-danger">
                    <h6 class="m-0 font-weight-bold text-white">
                        <i class="fas fa-ban"></i> {{ __('messages.Ban_This_User') }}
                    </h6>
                </div>
                <div class="card-body">
                    @if($user->activate == 2)
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-triangle"></i> <strong>{{ __('messages.Warning') }}:</strong> {{ __('messages.User_Already_Banned_Message') }}
                    </div>
                    @endif
                    ...
                    <form action="{{ route('users.ban', $user->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            <button type="submit" class="btn btn-danger btn-lg">
                                <i class="fas fa-ban"></i> {{ __('messages.Ban_User') }}
                            </button>
