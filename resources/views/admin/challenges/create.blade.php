...
@section('title', __('messages.Add_New_Challenge'))

@section('content')
<div class="container-fluid">
    <!-- Page Heading -->
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">{{ __('messages.Add_New_Challenge') }}</h1>
            ...
            <form action="{{ route('challenges.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description_en') is-invalid @enderror" 
                                      id="description_en" name="description_en" rows="3" required>{{ old('description_en') }}</textarea>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('challenge_type') is-invalid @enderror" 
                                ...
                                <option value="{{ $key }}" {{ old('challenge_type') == $key ? 'selected' : '' }}>
                                    {{ $type['en'] }} - {{ $type['ar'] }}
                                </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('target_count') is-invalid @enderror" 
                                   id="target_count" name="target_count" min="1" value="{{ old('target_count') }}" required>
                            @error('target_count')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('reward_amount') is-invalid @enderror" 
                                   id="reward_amount" name="reward_amount" step="0.01" min="0" value="{{ old('reward_amount') }}" required>
                            @error('reward_amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('max_completions_per_user') is-invalid @enderror" 
                                   id="max_completions_per_user" name="max_completions_per_user" min="1" value="{{ old('max_completions_per_user', 1) }}" required>
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> {{ __('messages.Create_Challenge') }}
                    </button>
                    <a href="{{ route('challenges.index') }}" class="btn btn-secondary">
